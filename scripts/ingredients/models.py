...
def init_lgm(gm_type, input_size, encoder_layers, latent_size,
    ...
    encoder_layers = [('unflatten', [-1, input_size])] + encoder_layers + \
                     [('linear', [2 * latent_size])]
    decoder_layers += [('flatten', [1])]
...
def load_lgm(path, device):
    path = os.path.join(path, str(id))

    meta = os.path.join(experiment_path, 'config.json')
    param_vals = os.path.join(experiment_path, 'trained-model.pt')
...
def init_pred(input_size, layers, n_targets):
    layers = [('unflatten', [-1, input_size])] + layers + \
             [('linear', [n_targets])]
...
def load_lgm_from_compnet(path, device):
    meta = os.path.join(path, 'config.json')
    param_vals = os.path.join(path, 'trained-model.pt')
